...
# %%
# !python -c "import pvporcupine; print('✅ Porcupine is ready!')"
...
# # Initialize Models
# # Whisper stays on CPU to keep your RTX 3050 free for Ollama
...
# def main():
#     print("🚀 JARVIS is in Passive Listening mode...")
#     recorder.start()

#     try:
#         while True:
#             # 1. Listen for Wake Word (Low Power)
#             pcm = recorder.read()
...
#                 # 2. Record a short 4s burst for the command
#                 frames = []
#                 for _ in range(0, int(recorder.sample_rate / recorder.frame_length * 4)):
#                     frames.extend(recorder.read())
                
#                 # 3. Process Command (High Power)
...
# def speak(text):
#     """Re-initializes engine for every response to prevent the 'one-speak' bug."""
#     print(f"🤖 JARVIS: {text}")
#     try:
#         # Re-init engine inside function
#         engine = pyttsx3.init()
#         engine.setProperty('rate', 180)
        
#         # Set voice (usually David/Male at index 0)
...
#         # Clean shutdown for the TTS driver
#         engine.stop()
#         del engine
#     except Exception as e:
...
# def main():
#     print(f"🚀 JARVIS is active. Say {WAKE_WORDS} to begin.")
#     recorder.start()
...
#             if keyword_index >= 0:
#                 play_chime()
#                 print(f"🔥 Wake word heard. Listening...")
                
#                 # 2. Record command (4s burst)
#                 frames = []
#                 for _ in range(0, int(recorder.sample_rate / recorder.frame_length * 4)):
#                     frames.extend(recorder.read())
...
#                 audio_data = np.array(frames, dtype=np.int16).astype(np.float32) / 32768.0
#                 segments, _ = stt_model.transcribe(audio_data, beam_size=1, vad_filter=True)
#                 command = " ".join([s.text for s in segments]).strip()

#                 if command:
#                     print(f"💬 Command: {command}")
#                     reply = get_ollama_response(command)
#                     speak(reply)  # Re-init happens here
                
#                 play_reset_beep()
#                 print(f"💤 Waiting for {WAKE_WORDS}...")
...
# def main():
#     print(f"🚀 JARVIS online. Say {WAKE_WORDS} to wake me.")
#     recorder.start()
...
#                 frames = []
#                 for _ in range(int(recorder.sample_rate / recorder.frame_length * 4)):
#                     frames.extend(recorder.read())
...
# engine = pyttsx3.init()
# engine.say("If you hear this, your system audio works.")
# engine.runAndWait()
...
#     """High-pitched beep to signal that JARVIS is now listening."""
...
#             print(f"[TTS ERROR] {e}")
#         finally:
#             # This marks the task as done, unblocking tts_queue.join()
#             tts_queue.task_done()
#     engine.endLoop()
#     pythoncom.CoUninitialize()
...
#     """
#     Sends text to the voice thread. 
#     If wait_for_finish is True, the main thread stops until JARVIS stops talking.
#     """
#     print(f"🤖 JARVIS: {text}")
#     tts_queue.put(text)
#     if wait_for_finish:
...
# def main():
#     print(f"🚀 JARVIS online. Say '{WAKE_WORDS[0]}' to begin.")
#     recorder.start()
...
#                 # 1. Stop recording while JARVIS greets you
#                 recorder.stop() 
#                 play_chime()
                
#                 # We set wait_for_finish=True so he finishes the sentence before we record
#                 speak("hello sir, what can i help you with today ?", wait_for_finish=True)
                
#                 # 2. Start listening now that he's done
#                 play_listening_beep()
#                 print("🎙️ Listening...")
#                 recorder.start()
                
#                 # B. Record 8s Command (Increased from 4s)
#                 frames = []
#                 # 8 seconds = recorder.sample_rate * 8
#                 for _ in range(int(recorder.sample_rate / recorder.frame_length * 8)):
#                     frames.extend(recorder.read())
                
#                 # 3. Stop recording to process
#                 recorder.stop()
#                 print("⏳ Processing...")
...
#                         speak("All i can say you is some Presentations and Calculations are still to be made, so lets say its okaish sir", wait_for_finish=True)
#                     else:
#                         reply = get_ollama_response(command)
#                         speak(reply, wait_for_finish=True)
                
#                 play_reset_beep()
#                 print("💤 Waiting for wake word...")
#                 recorder.start() # Resume waiting for 'Jarvis'
